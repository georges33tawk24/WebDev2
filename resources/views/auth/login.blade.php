<x-layouts.auth-split title="{{ __('ui.auth.login') }} — {{ __('ui.app_name') }}" heroVariant="login">
    ...
    <form method="POST" action="{{ route('login.attempt', [], false) }}" class="auth-form auth-form--split">
        @csrf

        <label class="field-label" for="email">{{ __('ui.auth.email') }}</label>
        <div class="input-shell @error('email') input-shell--error @enderror">
            ...
            <input id="email" type="email" name="email" value="{{ old('email') }}" placeholder="{{ __('ui.placeholders.email') }}" required autocomplete="username" @error('email') aria-invalid="true" @enderror>
        </div>
        ...
        <div class="input-shell @error('email') input-shell--error @enderror">
            ...
            <input id="password" type="password" name="password" placeholder="{{ __('ui.placeholders.password') }}" required autocomplete="current-password" @error('email') aria-invalid="true" @enderror>
